...
raw_sample = pd.read_csv(dataset_path + "/raw_sample.csv", sep = ",",encoding ="ISO-8859-1").drop(['time_stamp', 'pid', 'nonclk'], axis=1).dropna()
raw_sample.adgroup_id = raw_sample.adgroup_id.astype(int)
raw_sample.head()

user = pd.read_csv(dataset_path + "/user_profile.csv", sep = ",",encoding ="ISO-8859-1").dropna()
...
user_features = pd.concat([user.drop(columnsToEncode, axis=1),
                           pd.DataFrame(myEncoder.transform(user[columnsToEncode]), # type: ignore
...
ad_feature_ = pd.read_csv(dataset_path + "/ad_feature.csv", sep = ",",encoding ="ISO-8859-1").dropna()
...
ad_features = pd.concat([ad_feature.drop(columnsToEncode, axis=1),
                           pd.DataFrame(myEncoder.transform(ad_feature[columnsToEncode]), # type: ignore
                                        columns = myEncoder.get_feature_names_out(columnsToEncode))], axis=1).reset_index(drop=True)
...
os.makedirs(os.path.join(dataset_path, "preprocess"), exist_ok=True)
np.save(os.path.join(dataset_path, "preprocess", f"X0_{DATASET}.npy"), X[reward0_idx,:])
np.save(os.path.join(dataset_path, "preprocess", f"X1_{DATASET}.npy"), X[reward1_idx,:])
